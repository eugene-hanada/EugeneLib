...
{
	DirectX::XMFLOAT4 q{x, y, z, w};
	DirectX::XMStoreFloat4x4(&out,DirectX::XMMatrixRotationQuaternion(DirectX::XMLoadFloat4(&q)));
}

void Eugene::Quaternion::Look(const Vector3& dir, const Vector3& up)
{
	DirectX::XMFLOAT3 xmDir{dir.x, dir.y, dir.z};
	DirectX::XMFLOAT3 xmUp{up.x, up.y, up.z};
	DirectX::XMVECTOR scale, trans, q;
	DirectX::XMMatrixDecompose(
		&scale,
		&q,
		&trans,
		DirectX::XMMatrixLookToLH(DirectX::XMVectorZero(), DirectX::XMLoadFloat3(&xmDir), DirectX::XMLoadFloat3(&xmUp))
	);
	DirectX::XMFLOAT4 resultQ;
	DirectX::XMStoreFloat4(&resultQ, q);
	x = resultQ.x;
	y = resultQ.y;
	z = resultQ.z;
	w = resultQ.w;
