...
Eugene::VkImageResource::VkImageResource(const VkGraphics& graphics, const vk::Device& device, Vector2I& size, float clearValue) :
	ImageResource{Format::D32_FLOAT}
{
	...
	imageInfo.extent.setHeight(size.x);
	imageInfo.extent.setWidth(size.y);
	imageInfo.setTiling(vk::ImageTiling::eOptimal);
	imageInfo.setInitialLayout(vk::ImageLayout::eUndefined);
...
{
	return static_cast<void*>(const_cast<Data*>(&data_));
}
